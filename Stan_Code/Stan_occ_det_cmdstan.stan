...
data {
  ...
  int<lower=0> K_fo; // Number of fixed  variables Occupancy
  int<lower=0> K_ro; // Number of random  variables Occupancy
  int<lower=0> K_fd; // Number of fixed  variables Detectability
  int<lower=0> K_rd; // Number of random  variables Detectability
  
  int<lower=1> L_year;  // Number of years
  int<lower=1> L_area; // number of areas for which separate random walks are estimated
  int<lower=1> L_ro[K_ro]; // Number of levels of random variables at occupany level
  int<lower=1> L_rd[K_rd]; // Number of levels of random variabless at detectability level

  int x_year[P, 1]; // year of observation
  int x_area[P, 1]; // area of observation
  matrix[P, K_fo] x_fo; // fixed variables Occupancy
  int x_ro[P, K_ro]; // random variables Occupancy CONVERT LEVELS TO INTEGERS
  matrix[N, K_fd] x_fd; // fixed variables Detectability 
  int x_rd[N, K_rd]; // random variables Detectability CONVERT LEVELS TO INTEGERS
...
parameters {
  // global intercept Occupancy
  real mu_o; 

  // year effect Occupancy
  matrix[L_year, L_area] alpha_year;
  vector<lower=0,upper=100>[L_area] sigma_a_year;

  // random effects Occupancy
  vector[max_L_ro] alpha_ro[K_ro];
  vector<lower=0,upper=100>[K_ro] sigma_a_ro;
  
  // fixed effects Occupancy
  vector<lower=-10,upper=10>[K_fo] beta_fo; 
  
  // global intercept Detectability
  real mu_d;

  // random effects Detectability
  vector[max_L_rd] alpha_rd[K_rd];
  vector<lower=0,upper=100>[K_rd]  sigma_a_rd;

  // fixed effects Detectability
  vector<lower=-10,upper=10>[K_fd] beta_fd;
}
...
transformed parameters {
  ...
  vector[N] logit_p_v; // Logit detection probability
  ...
  if (K_fd > 0) { // check whether there are actually any covariates in the detectability model
...
model {
  ...
  target += reduce_sum(partial_sum_psi, V[occ], // V here just a dummy
                       grainsize1,
                       logit_psi[occ]);
